...
export function IncomeDistribution() {
  const { income, payday, incomeDistribution, setIncome, setPayday, setIncomeDistribution } = useStore()
  ...
  const totalPercentage = Object.values(incomeDistribution).reduce((sum, value) => sum + value, 0)

  return (
    <Card className="col-span-full lg:col-span-3">
      <CardHeader>
        <CardTitle className="text-2xl font-bold">Income Distribution</CardTitle>
        ...
        {!isEditing ? (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <div className="space-y-1">
                <p className="text-sm font-medium text-muted-foreground">Payday</p>
                <p className="text-2xl font-bold">{payday}<sup>th</sup></p>
              </div>
            </div>
            <Separator />
            <div className="space-y-4">
              <h4 className="font-semibold text-lg">Distribution Breakdown</h4>
              {Object.entries(incomeDistribution).map(([balance, percentage]) => (
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              {Object.entries(tempDistribution).map(([balance, percentage]) => (
                <div key={balance} className="space-y-2">
                  <Label htmlFor={`distribution-${balance}`} className="flex justify-between">
                    <span>{balance}</span>
                    <span className={percentage > 100 || totalPercentage > 100 ? "text-red-500" : ""}>{percentage}%</span>
                  </Label>
                  <Input
                    ...
                    max="100"
                    value={percentage}
                    onChange={(e) => setTempDistribution({...tempDistribution, [balance]: Number(e.target.value)})}
                    className="w-full"
                  />
                </div>
              ))}
              {totalPercentage > 100 && (
                <p className="text-sm text-red-500">Total percentage exceeds 100%. Please adjust your distribution.</p>
              )}
            </div>
            <div className="flex space-x-2 pt-4">
              <Button type="submit" className="flex-1" disabled={totalPercentage > 100}>
